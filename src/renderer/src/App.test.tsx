...
<score-partwise>
  ...
  it('passes onOpenSettings/onOpenGlove handlers to LibraryView (header hidden fallback)', async () => {
    render(<App />);

    await waitFor(() => expect(latestLibraryViewProps?.onOpenEntry).toBeInstanceOf(Function));
    expect(latestLibraryViewProps.onOpenSettings).toBeInstanceOf(Function);
    expect(latestLibraryViewProps.onOpenGlove).toBeInstanceOf(Function);
  });
