...
export const LibraryView: React.FC<LibraryViewProps> = ({
  ...
  // ライブラリ画面ではヘッダーが非表示のため、トップバーにファイル・手套・設定の
  // カプセルボタンを配置する。手套ボタンは接続状態に応じて緑点を表示する。
      ...
      <div className="kf-library__topbar-actions">
        ...
        {onOpenGlove && (
          <button
            ...
            className={`kf-pill-btn ${isGloveConnected ? 'kf-pill-btn--active' : ''}`}
            style={{ position: 'relative' }}
          >
            <GloveIcon />
            <span>{t.header.gloveButtonTitle}</span>
            {isGloveConnected && (
              <span
                data-testid="library-glove-connection-dot"
                aria-hidden="true"
                className="kf-connection-dot"
              />
            )}
          </button>
        )}
