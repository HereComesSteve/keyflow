import type { Messages } from './types';

/**
 * 英語リソース（TASK-096/097/098）。`Messages`型（`ja`の構造から導出）への適合を
 * 型チェックで強制する。キーの欠落・タイポはコンパイルエラーになる（DEC-009）。
 */
export const en: Messages = {
  settings: {
    title: 'Settings',
    closeButtonAriaLabel: 'Close',
    doneButton: 'Done',
    cancelButton: 'Cancel',
    saveButton: 'Save',
    loadError: 'Failed to load settings. Showing defaults.',
    saveError: 'Failed to save settings. Reverted the change.',
    practiceSectionTitle: 'Practice',
    errorModeLabel: 'Default error mode',
    errorModeWait: 'Wait for the correct note',
    errorModePass: 'Move on even if there are mistakes',
    metronomeEnabledLabel: 'Enable metronome by default',
    metronomeAccentEnabledLabel: 'Accent the first beat by default',
    displaySectionTitle: 'Display',
    pianoHeightLabel: 'Keyboard height',
    pianoHeightTitle: 'Change the display height of the piano keyboard (80-300px)',
    keyboardSizeLabel: 'Number of keys',
    keyboardSizeTitle: "Change the keyboard's displayed key range to match your device",
    keyboardSizeOption88: '88 keys (full-size piano)',
    keyboardSizeOption76: '76 keys',
    keyboardSizeOption61: '61 keys',
    keyboardSizeOption49: '49 keys',
    language: 'Language',
    languageTitle: 'Switch the display language (applies immediately, no restart needed)',
    languageOptionJapanese: '日本語',
    languageOptionEnglish: 'English',
    midiSectionTitle: 'MIDI',
    midiDeviceLabel: 'MIDI input device',
    midiDeviceTitle: 'Select the MIDI input device to use (when multiple are connected)',
    midiAllDevices: 'All devices',
    voiceSectionTitle: 'Voice',
    playbackVoiceLabel: 'Playback voice',
    playbackVoiceTitle: 'Select the voice used for song playback and manual preview',
    metronomeVoiceLabel: 'Metronome voice',
    metronomeVoiceTitle: 'Select the metronome click sound',
    recentFilesSectionTitle: 'Recent files',
    recentFilesEmpty: 'No recent files',
  },
  about: {
    dialogAriaLabel: 'About',
    closeButtonAriaLabel: 'Close',
    librariesHeading: 'Libraries used',
    noLibraries: 'No library information available',
  },
  app: {
    dropHint: 'Drop a MusicXML file here (or open a file)',
    unsupportedDropFormat: 'Unsupported file format. Please drop a .xml / .musicxml / .mxl file.',
    electronApiUnavailable: 'Electron API is unavailable. Please run this as an Electron app.',
    parseError: 'Failed to parse the MusicXML file. Please check the file format.',
    loadingScore: 'Loading score...',
    openDialogError: 'Failed to open the file selection dialog.',
    droppedFilePathError: 'Failed to resolve the path of the dropped file.',
    libraryOpenError: 'Failed to open the library entry.',
    fingeringSaveError: 'Failed to save the fingering annotations.',
    annotationSaveError: 'Failed to save the fingering note.',
  },
  voiceNames: {
    grandPiano: 'Grand Piano',
    electricPiano: 'Electric Piano',
    organ: 'Organ',
    synth: 'Synth',
    click: 'Click',
    woodblock: 'Woodblock',
    beep: 'Beep',
    cowbell: 'Cowbell',
  },
  header: {
    openFileAriaLabel: 'Open file',
    openFileTitle: 'Open a MusicXML file',
    quickPanelAriaLabel: 'View & tools',
    quickPanelTitle: 'View & tools (volume, zoom, fingering, stats)',
    libraryButtonAriaLabel: 'Library',
    libraryButtonTitle: 'Show the library screen',
    returnToScoreAriaLabel: 'Back to score',
    returnToScoreTitle: 'Return to the score view',
    settingsAriaLabel: 'Settings',
    settingsTitle: 'Settings',
    gloveButtonAriaLabel: 'Bluetooth glove',
    gloveButtonTitle: 'Connect a vibration glove',
  },
  quickPanel: {
    displaySection: 'Display',
    fingeringSection: 'Fingering',
    statsSection: 'Stats',
    metronomeDetailSection: 'Metronome details',
  },
  metronome: {
    toggleLabel: 'Metronome',
    accentLabel: 'Accent 1st beat',
    accentTitle: 'Play the first beat of the metronome louder than the others',
  },
  playbackControls: {
    play: 'Play',
    playTitle: 'Play (Space)',
    pause: 'Pause',
    pauseTitle: 'Pause (Space)',
    stop: 'Stop',
    stopTitle: 'Stop',
    noScoreTooltip: 'Open a score to play',
    voiceLoadingTooltip: 'Loading voice...',
    voiceLoadingLabel: 'Loading...',
    startError:
      'Playback could not be started. Check that an audio output device is available (speakers or headphones connected, and not held exclusively by another app), then try again.',
    rangePlaceholder: '1-3, 1-5',
    rangeTitle: 'Playback measure range (empty = all)',
    clearButton: 'Clear',
    clearTitle: 'Clear the current playback range (switch to linear playback)',
    resetButton: 'Reset',
    resetTitle: 'Reset the playback range from the score repeat marks',
    loopButton: 'Loop',
    loopButtonTitle: 'Open the playback loop panel',
    loopPanelTitle: 'Playback loop range',
    closeButton: 'Close',
  },
  tempoControl: {
    bpmIconTitle: 'Tempo (BPM: beats per minute)',
    inputTitle: 'Set the tempo directly in BPM (beats per minute)',
    sliderTitle: 'Tempo (percentage of the original tempo, 20%-200%)',
    resetTitle: 'Reset the tempo to the score original tempo',
    resetAriaLabel: 'Reset tempo',
  },
  practiceModeSelector: {
    groupTitle:
      'Switch the practice target (left/right/both hands). Separate from the fingering target selection',
    left: 'Left (L)',
    leftTitle: 'Practice the left hand only (shortcut: L)',
    right: 'Right (R)',
    rightTitle: 'Practice the right hand only (shortcut: R)',
    both: 'Both (B)',
    bothTitle: 'Practice both hands (shortcut: B)',
  },
  volumeControl: {
    label: 'Volume:',
    title: 'Adjust the master volume (0 mutes playback, metronome, and sound effects)',
  },
  zoomControl: {
    label: 'Zoom:',
    title: 'Change the score display zoom level',
  },
  scoreLayoutControl: {
    label: 'Layout:',
    title: 'Switch the score page layout between vertical and horizontal',
  },
  fingeringToggle: {
    label: 'Fingering',
    titleShow: 'Show fingering numbers on the score and keyboard',
    titleHide: 'Hide fingering numbers on the score and keyboard',
    statusShown: 'Shown',
    statusHidden: 'Hidden',
  },
  fingeringEditToggle: {
    label: 'Edit fingering',
    titleOn: 'Fingering edit mode is ON. Click a fingering number on the score to change it.',
    titleOff: 'Enter fingering edit mode',
    statusOn: 'ON',
    statusOff: 'OFF',
  },
  fingeringPicker: {
    title: 'Fingering',
  },
  statsDisplay: {
    accuracyLabel: 'Accuracy:',
    consecutiveLabel: 'Streak:',
  },
  fingeringPanel: {
    handSelectLabel: 'Fingering target:',
    handOptionRight: 'Right hand',
    handOptionLeft: 'Left hand',
    handSelectTitle:
      'Select the hand to compute fingering for (separate from the practice target setting)',
    computeButton: 'Suggest fingering',
    noNotesError: '{hand}: no notes found',
  },
  noteContextMenu: {
    title: 'Fingering note: {noteId}',
    fingerNumberLabel: 'Finger number',
    removeFingerButton: 'Remove finger number',
    commentLabel: 'Comment',
    saveCommentButton: 'Save comment',
    approveButton: 'Approve AI suggestion',
    closeButton: 'Close',
  },
  scoreRenderer: {
    placeholder: 'Open a score file to begin',
  },
  library: {
    title: 'Library',
    searchLabel: 'Search',
    searchPlaceholder: 'Search by title or composer',
    sortKeyLabel: 'Sort by',
    sortKeyTitle: 'Title',
    sortKeyAddedAt: 'Added date',
    sortKeyLastOpenedAt: 'Last opened',
    sortOrderLabel: 'Order',
    sortOrderAsc: 'Ascending',
    sortOrderDesc: 'Descending',
    columnTitle: 'Title',
    columnComposer: 'Composer',
    columnLastOpenedAt: 'Last opened',
    columnActions: 'Actions',
    deleteButton: 'Delete',
    deleteButtonAriaLabel: 'Remove {title} from the library',
    confirmDeleteTitle: 'Remove from library',
    confirmDeleteMessage:
      'This will remove "{title}" from the library. The original file will not be deleted.',
    confirmDeleteConfirmButton: 'Remove',
    confirmDeleteCancelButton: 'Cancel',
    noResults: 'No matching scores found',
    emptyTitle: 'Your library is empty',
    emptyDescription: 'Scores are added to the library automatically when you open them.',
    emptyOpenFileButton: 'Open file',
    loadErrorTitle: 'Failed to load the library',
    loadErrorDescription: 'Please try again.',
    retryButton: 'Retry',
    deleteErrorMessage: 'Failed to remove "{title}".',
    missingLabel: 'Missing',
    missingTitle: 'File not found (it may have been moved or deleted)',
    missingEntryErrorMessage:
      'The selected score file was not found. It may have been moved or deleted.',
    missingEntryConfirmTitle: 'Remove from library',
    missingEntryConfirmMessage:
      'Remove this file from the library? The original file location is not changed.',
    missingEntryRemoveErrorMessage: 'Failed to remove from the library. Please try again.',
    returnToScoreButton: 'Back to score',
  },
  glove: {
    dialogTitle: 'Bluetooth glove connection',
    closeButtonAriaLabel: 'Close',
    leftGloveLabel: 'Left glove (Piano_L)',
    connectButton: 'Connect',
    disconnectButton: 'Disconnect',
    statusLabel: 'Status:',
    statusConnected: 'Connected',
    statusDisconnected: 'Not connected',
    scanPrompt: 'Select a device to connect',
    scanningHint: 'Scanning for BLE devices...',
    noDevicesYet: 'No devices found yet',
    cancelScan: 'Cancel scan',
    playbackSection: 'Playback control',
    sramPlay: 'SRAM play',
    eepromPlay: 'EEPROM play',
    stop: 'Stop',
    pause: 'Pause',
    resume: 'Resume',
    reset: 'Reset',
    partitionLabel: 'Partition',
    modeSection: 'Mode',
    singleHand: 'Single-hand',
    dualHand: 'Dual-hand',
    targetSection: 'Target device',
    targetLeft: 'Left',
    targetRight: 'Right',
    logSectionTitle: 'Logs',
    logEmpty: 'No logs',
    logScanning: 'Scanning...',
    logConnected: 'Connected to {device}',
    logDisconnected: 'Disconnected',
    logConnectError: 'Connection failed: {error}',
    logDisconnectError: 'Disconnection failed: {error}',
    logUnavailable: 'Web Bluetooth API is unavailable',
    logCancelled: 'Device selection cancelled',
    logScanTimeout: 'Device selection timed out (60s). Make sure the glove is powered on and in pairing mode',
    logNotConnected: 'Please connect the glove first',
    logSent: 'Sent {cmd} ({desc})',
    logSendError: 'Send failed: {error}',
    writeSection: 'Score writing',
    writeTargetLabel: 'Target device',
    writeStorageLabel: 'Storage',
    sram: 'SRAM',
    eeprom: 'EEPROM',
    scoreDataLabel: 'Score data (hex, tick motor pairs)',
    scoreDataPlaceholder: '00 10 20 22 30 01',
    writeButton: 'Write score',
    writingButton: 'Writing...',
    cancelWriteButton: 'Cancel',
    writingProgress: 'Sending... {current}/{total}',
    statCommands: '{count} commands',
    sramCapacityHint: 'SRAM supports up to 32 commands',
    eepromCapacityHint: 'EEPROM partition capacity: ~2000 commands',
    errEmpty: 'No data entered',
    errOddBytes: 'Data format error: must be an even number of bytes',
    errInvalidHex: 'Invalid hex byte: {value}',
    errSramOverflow: 'SRAM supports up to 32 commands, current {count}, excess will be discarded',
    logWriteStart: 'Start writing score (target: {target}, storage: {storage})',
    logWriteTargetLeft: 'Target -> Left',
    logWriteTargetRight: 'Target -> Right',
    logWriteClearSram: 'Clear SRAM',
    logWriteClearEeprom: 'Clear EEPROM partition {partition}',
    logWriteInitEeprom: 'Enter EEPROM write mode (partition {partition})',
    logWriteScore: 'Score #{index}',
    logWriteEnd: 'End marker',
    logWriteComplete: 'Write complete! Sent {count} commands',
    logWriteTotalTime: 'Total time: {seconds}s',
    logWriteCancelled: 'Write cancelled (sent {sent}/{total} commands)',
    logWriteError: 'Write failed: {error}',
    bpmSection: 'BPM setting',
    bpmInputLabel: 'BPM',
    bpmSetButton: 'Set',
    bpmCurrent: 'Current: {value} BPM',
    bpmRangeError: 'BPM range: 1~300',
    timeSignatureSection: 'Time signature',
    timeSignatureLabel: 'Time signature',
    timeSignatureSetButton: 'Set',
    timeSignatureCurrent: 'Current: {value}',
    intensitySection: 'Motor intensity',
    intensityFingerLabel: 'Finger',
    intensityValueLabel: 'Intensity',
    intensitySendButton: 'Send',
    intensitySendAllButton: 'Send all',
    intensityRangeError: 'Intensity range: 1~255',
    intensityHint:
      'Command: F9 25 <finger 1-5> <intensity 1-255> (fixed 4 bytes, no XOR). Select the target hand first; the command is not auto-forwarded in dual mode so each hand can be set independently.',
    logSetIntensity: 'Set finger {finger} intensity to {value} ({hand})',
    jumpSection: 'Jump to bar',
    jumpBarLabel: 'Bar',
    jumpButton: 'Jump',
    jumpRangeError: 'Bar range: 1~255',
    jumpHint:
      'Command: F9 26 <bar 1-based> 00 (fixed 4 bytes, no XOR). Select the target hand first; the currently playing score (SRAM/EEPROM) jumps to that bar and resumes from its start.',
    logJumpBar: 'Jump to bar {value} ({hand})',
    clearSection: 'Clear score',
    clearTargetLabel: 'Clear target',
    clearPartitionLabel: 'EEPROM partition',
    clearButton: 'Clear',
    clearWarning: 'This action cannot be undone',
    clearConfirmTitle: 'Confirm clear',
    clearConfirmMessage: 'Are you sure you want to clear? This action cannot be undone.',
    clearConfirmOk: 'Confirm',
    clearConfirmCancel: 'Cancel',
    logSetBpm: 'Set BPM to {value}',
    logSetTimeSignature: 'Set time signature to {value}',
    importFromScore: 'Import from Current Score',
    importFromScoreHint: 'Generate commands from current score and fingering',
    rangeLabel: 'Range',
    rangePlaceholder: '1-3, 5-7, 9 (empty=all)',
    rangeHint: 'Comma-separated. Supports 1-3, 5-7, 9 format. Supports repeat, e.g. 1-4, 1-4 to practice first 4 measures twice.',
    noScoreLoaded: 'Please load a score first',
    noFingeringData: 'Please calculate fingering first',
    noNotesForHand: 'No notes found for {hand} hand',
    importSuccess: 'Imported {notes} notes → {instructions} commands',
    sramCapacityWarning: 'SRAM supports max 32 instructions, current {count}. Excess will be discarded.',
    importStatus: 'Imported from score',
  },
};
