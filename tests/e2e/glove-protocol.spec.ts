...
test('串行验证：连接→BPM→SRAM→播放→EEPROM', async () => {
  ...
  for (let attempt = 0; attempt < 5 && !clicked; attempt++) {
    await window.locator('.glove-scan__item').first().waitFor({ timeout: 15_000 });
    ...
    console.log(`[2] 扫描第 ${attempt + 1} 轮:`, JSON.stringify(names));
    if (target >= 0) {
  ...
  const okWrite = !/NAK|failed|失敗/i.test(sramLogs) && !/CRC\s*(fail|mismatch|err|bad)/i.test(sramLogs);
  ...
  const okEeprom = !/NAK|failed|失敗/i.test(eepromLogs) && !/CRC\s*(fail|mismatch|err|bad)/i.test(eepromLogs);
